...
| B1-CRYPTO-06 | phase-91 | LOW | `DatabaseSecurityHelper.kt:146-154` verifyDatabaseIntegrity: stored==null -> `updateStoredChecksum(context); return... | `gradle testDebugUnitTest` 1577 green + `gradle assembleDebug` green — `FIXED` 2026-08-16 (see `workspace/phase-91/REPORT.md`): fail-closed `DatabaseIntegrityVerdict` via `services/DatabaseIntegrityPolicy.kt`, no writes/no re-baseline, distinct per-session "cannot verify" notice, fresh-vault-only auto-arm. Review fixes (2026-08-16): WAL-aware fresh-vault probe (populated `-wal` = existing vault; empty-main + populated WAL stays hashable), passwordless first-verification deferred past the initial open (no false-Mismatch on WAL recovery), `CannotVerify` honors the per-session dismissal gate, banners deduped into one `IntegrityBannerCard` — `B1Crypto06DatabaseIntegrityPolicyTest` 14 + `B1Db06` 16, `gradle testDebugUnitTest` 1580 green |
