...
- `OneEuroParityTest` (9): key sanitization fail-safe (incl. path-traversal string), facade model reporting, bit-identical EWMA after model round-trip, no-op reselect keeps warm-up, constants + beta mapping goldens, exact steady-state convergence, standstill jitter collapse (<25 % spread), fast-ramp lag < 75 % of static window-8 EWMA, SettingsManager pref pins.
- `Phase214StrokeSmoothingV2Test` (12): fairing gate matrix, endpoint preservation + `2n−2` size, turn-angle reduction, channel interpolation (pressure/tilt/timestamp), cap-refusal, degenerate inputs, Long-exact timestamp interpolation beyond Float precision (review-fix regression pin), commit-order source pins (simplify → shouldFair → chaikinOnce → capLoadedPoints, single simplify call site, no fairing on the snapped branch), stroke-start model/tension wiring pins, sheet UI pins, sheet strings-in-resources pin, tension policy goldens.

**Total: 45 new `@Test` methods** (43 at the phase commit + 2 review-fix pins). Updated: `Phase197StrokeSmoothingTest` (1 pin), `Phase196MotionPredictionTest` (1 pin) — see §5.

Full suite: at the phase commit, `gradle testDebugUnitTest` = **3197 completed / 3 failed**, all pre-existing environmental (Phase148 UNC-path documented in AGENTS.md; PaparazziSmokeTest ×2 layoutlib). **Review re-run (2026-08-26): 3197 / 4 failed** — the same three PLUS `Phase151MarkdownMainThreadPerfTest`, a known timing/concurrency flake per AGENTS.md that was verified to PASS IN ISOLATION during the review (not a phase-214 regression; the original "identical baseline" phrasing understated the runner-dependent flake set). Post-review-fix re-run: see §9.
...
## 9. Review fixes (2026-08-26, all 7 findings actioned)

1. **Test inventory corrected** — §6 originally claimed 42 tests split 9/8/9/8/8; the actual JUnit-verified counts were 43 split 8/7/9/9/10. §6, `docs/ARCHITECTURE.md` and `docs/phase-status.md` now carry exact counts.
2. **Failure manifest corrected** — the "3 failed, identical to baseline" claim was runner-dependent: the review re-run surfaced a fourth failure (`Phase151MarkdownMainThreadPerfTest`, known timing/concurrency flake per AGENTS.md, passes in isolation — verified, not a regression). §6 now states both runs honestly.
3. **`StrokeInputBatcher` thread-safety claim scoped honestly** — the overflow path advances the consumer-owned `head` from the producer side, so genuinely concurrent producer/consumer use was never safe despite the old KDoc ("safe even if … different threads"). The class doc now states the supported contract: both sides on the UI thread today; volatile indices publish slots for that case only.
4. **Monotonic gate no longer advances past rejected samples** — `ingestPointerSample` returns `Boolean` (false ONLY on the page-bounds rejection) and all three call sites update `lastIngestedInputTimestampMs` only when the sample was actually accepted ("last ACCEPTED" now means accepted). Pinned in `HistoryBatchTest`.
5. **Fairing timestamps interpolate Long-exact** — `mixPoint` blends `timestampMs` in DOUBLE space with `roundToLong`; eventTime millis exceed Float's 24-bit exact range after ~4.6 h uptime, where the old Float blend could round stamps coarse/duplicate (androidx.ink consumes per-point tMs). Regression-pinned at 2·10¹⁰ ms magnitude.
6. **Capture hot-path allocations trimmed** — velocity now uses an allocation-free scalar `BrushStrokeMath.segmentVelocity(x,y,ts,…)` overload (PointF version delegates to it — byte-identical math, existing `BrushStrokeMathTest` green); `drainInto`'s allocation story documented at the API (ring storage stays primitive; draining materializes ≤64 lightweight samples/frame). The `ingestPointerSample` closure capture is left intentionally — hoisting it out of `onDrag` needs a ~15-parameter signature for zero measurable gain.
7. **New sheet strings extracted to resources** — `canvas_tension_label/_percent_format/_helper`, `canvas_model_classic/_one_euro/_model_apply_note` live in `strings.xml`; EditorScreen uses `stringResource(...)` (pre-existing neighbouring rows keep their legacy literals — out of phase scope). Pins updated + new resources pin.

Post-fix verification: targeted suites re-run green (HistoryBatch 8, PressureSmoothing 7, VelocityAdaptiveAlpha 9, OneEuroParity 9, Phase214StrokeSmoothingV2 12 = **45**); full suite re-run = **3199 / 4 failed**, the SAME runner-dependent manifest as the review run (Phase148 UNC-path + PaparazziSmokeTest ×2 environmental; `Phase151MarkdownMainThreadPerfTest` timing flake — passes in isolation, re-verified post-fix); `gradle assembleDebug` green.
