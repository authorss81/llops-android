...
`.md` suffix consumed width that a daily-note title shouldn't show.
...
- After: `2026-08-19.md` → `2026-08-19`, single line; any longer title ellipsizes
  cleanly at 2 lines with no mid-word hyphenation, `.md`/`.markdown`/`.txt` hidden.
...
- `gradle testDebugUnitTest` — **2437 tests, 1 failed** = the pre-existing
  `Phase148UiFailureTextScrubTest` UNC-path failure (`\\fileserver\share\secret-wills.docx`
  assert), untouched and reproduced as documented. Baseline phase-182: 2426 tests /
  1 pre-existing failure; +11 new = 2437. ✓
