...
/**
 * B1-CRYPTO-04 (phase-63): the single source of truth for how strong a NEW
 * master password must be at `setMasterPassword` / `changeMasterPassword`.
 *
 * Threat model (the finding): the salt + wrapped DEK (`settings.masterPasswordSalt`
 * / `masterPasswordWrappedDek`, now the single `MPB1|...` blob, B1-CRYPTO-03) and
 * the SQLCipher `noteflow.sqlite` sit on the normal data partition. An attacker
 * who obtains a data copy (cloud/manual backup, forensic extraction, shared
 * device image) cracks the wrapped DEK OFFLINE with a GPU/FPGA PBKDF2-SHA-256
 * rig — the only on-device throttle is the UI lockout, which by definition never
 * fires for an offline attacker. A 6-7 character lowercase/numeric password falls
 * in hours-to-days. The on-device 5-fail lockout (`NoteflowViewModel` failed-
 * attempt counters) is UI-only: it cannot protect a copied vault.
 *
 * This policy therefore enforces a *stronger minimum* at password SET and
 * ROTATE time (never at verify/unlock — an existing 6-char vault must keep
 * unlocking):
 *  1. length ≥ [MIN_STRENGTH_GRAPHEMES] (8 perceived characters), ≤
 ...
 *     `12345678`, `abcdefgh`, `qwerty`, `aaaaaaaa`, `12121212` — patterns a
 *     GPU wordlist exhausts first);
 *  3. character-class diversity — a short password must mix at least 3 of the 4
 ...
 * human attempts through the UI. The vault's real protection against an offline
 * data copy is the password itself (plus the per-device DEK placement from
 * B1-CRYPTO-02). A TEE-bound attempt gate or an Argon2id KDF would raise the
 * offline cost; both are tracked as follow-ups (see `workspace/phase-63/REPORT.md`)
 * and are deliberately NOT introduced here (no new dependencies, no new
 * platform requirements — this policy runs on the API 26+ floor unchanged).
 *
...
internal enum class PasswordStrengthVerdict(val accepted: Boolean, val message: String) {
    ...
    SEQUENTIAL(
        false,
        "Password must not be a predictable pattern (e.g. 12345678, qwerty, abcdefgh)"
    ),
    WEAK(
        false,
...
internal object PasswordStrengthPolicy {
    ...
    const val MIN_STRENGTH_GRAPHEMES = 8
