 ...
 * the highest stroke page and the highest sticky-note / media-embed page — never
 * from how many bitmaps happen to be cached in the visible window. The export
 * loop then renders EVERY `0 until count` index with a background: the
 * pre-rendered cached bitmap when present, else the per-page source re-render,
 * else the page template — so no page can ever come out blank.
 */
object DocumentPdfExportPolicy {
     ...
     * zero-based) and one beyond the highest sticky-note / media-embed page
     * (`maxItemPageToExport` = notes/embeds' `pdfPage` levels, zero-based), each
     * capping at a floor of 1. The windowed bitmap cache size is deliberately NOT
     * an input — it undercounts by construction.
     */
    fun pageCountForExport(
        sourcePdfTotalPages: Int,
        maxStrokePageToExport: Int,
        maxItemPageToExport: Int = 0
    ): Int {
        val strokesRequired = maxOf(0, maxStrokePageToExport + 1)
        val itemsRequired = maxOf(0, maxItemPageToExport + 1)
        return maxOf(1, sourcePdfTotalPages, strokesRequired, itemsRequired)
    }
}
