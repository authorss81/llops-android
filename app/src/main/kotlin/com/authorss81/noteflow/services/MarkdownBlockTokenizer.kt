...
object MarkdownBlockTokenizer {
    ...
    /**
     * Line index (into [lines]) that CONTAINS the byte offset [byte] of
     * `lines.joinToString("\n")`. A byte pointing at a `\n` separator still
     * belongs to the preceding line; the end-of-content byte belongs to the
     * last line.
     */
    private fun lineIndexAtByte(lines: List<String>, byte: Int): Int {
        var off = 0
    ...
    /**
     * Phase 243 — replace the CONTIGUOUS CONTENT RUN [startByte, endByte) with
     * [newSource] and re-tokenize ONLY the affected window.
     *
     * The hybrid editor's keystroke path is anchored to a BYTE RUN, not a block
     * index: a keystroke can re-split the edited block's window into a different
     * block topology, and a subsequent block-index replace then rewrites a
     * drifted slot while the old fragments survive — the text-duplication bug.
     * Locating the previous edited source by text and replacing exactly those
     * bytes keeps the patient's neighbours intact no matter how the window
     * re-splits. This builds on the same cached-lines / window re-classify /
     * incremental-candidates machinery as [replaceBlock], so the produced
     * [MarkdownDocument] is byte-identical to a fresh full [tokenize] of
     * `content.replaceRange(startByte, endByte, newSource)` (unit-tested) while
     * only the window is re-scanned. An empty run (endByte <= startByte) is a
     * no-op.
     */
    fun replaceContentRun(
        ...
        out.addAll(newLines)
        out.addAll(src.subList(l1 + 1, n))
        val content = out.joinToString("\n")
        if (out.isEmpty()) {
            return MarkdownDocument(content, out, emptyList(), emptyList(), emptyMap())
        }
        if (out.all { it.isBlank() }) {
            // Entirely blank output — mirrors [blocksFromLines]'s all-blank
            // document block (a full re-tokenize emits exactly one such block).
            return MarkdownDocument(
                content, out,
                listOf(MarkdownBlock(MarkdownBlockType.PARAGRAPH, 0, out.size - 1)),
                emptyList(), emptyMap()
        ...
        val delta = newLines.size - (l1 - l0 + 1)

        // Affected window: the run plus the region that follows it (its first
        // block's grown start can move with a trailing blank run), and — when no
        // blank separates the run from the preceding region — the whole shared
        // region, so a merge is re-classified too. Mirrors [replaceBlock].
        val windowStart = if (l0 < out.size && !out[l0].isBlank()) {
            var w = l0
            while (w > 0 && !out[w - 1].isBlank()) w--
            w
        } else {
        ...
        val afterContent = firstNonBlankAtOrAfter(out, l0 + newLines.size)
        val windowEnd = if (afterContent < 0) out.size - 1 else {
            val nextBlank = firstBlankAfter(out, afterContent)
            if (nextBlank < 0) out.size - 1 else nextBlank - 1
        }
        ...
        // Range-based before/after (replaceBlock's index-based split with the
        // run possibly spanning several blocks): blocks wholly before the window
        // are byte-unchanged; blocks wholly after it only shift by the delta.
        // Everything inside the window — including the run itself — is
        // regenerated by the window pass.
        val before = doc.blocks.filter { it.endLine < windowStart }
        val beyond = doc.blocks.filter { it.startLine + delta > windowEnd }.map { b ->
            MarkdownBlock(b.type, b.startLine + delta, b.endLine + delta)
        }
        val editedIndex = doc.blocks.indexOfFirst { it.endLine >= windowStart }.let {
            if (it < 0) 0 else it
        }
        val candidates = incrementalCandidates(
            doc, out, editedIndex, before.size, windowBlocks, windowEnd, delta
        )
        return MarkdownDocument(content, out, before + windowBlocks + beyond, candidates, candidatesByBlock(candidates))
    }
