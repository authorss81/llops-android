...
/**
 * The real, on-device translator: Google ML Kit `translate`, moved out of the
 * base APK in phase-175 (R2-KS-21).
 *
 * PLATFORM-ONLY — talks to ML Kit's `Translation` client; cannot run in a JVM
 * unit test. Reached only from [OnDeviceMlKitPlugin] when no fake engine is
 * injected.
 *
...
internal object PlatformTranslationEngines {
    fun create(context: android.content.Context): TranslatorEngine? {
        val missing = PluginPayloadLoader.ensureNativeLibraries(context)
        return if (missing != null) null else MlKitTranslatorEngine()
    }
}