...
/**
 * THE interface between [OnDeviceMlKitPlugin] and the on-device translator —
 * the injected-engine pattern (moved from the base app in phase-175). The
 * plugin's wrapper logic (catalogue, target validation, source auto-detection)
 * is pure JVM and unit-tested with a fake engine; only [MlKitTranslatorEngine]
 * touches ML Kit's `Translation` client.
 */
