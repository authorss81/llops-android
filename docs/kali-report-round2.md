...
> **Provenance note (phase-160 review fix):** the at-hand APK's SHA-256 does NOT
> match phase-159's recorded artifact. That divergence is documented above, but
> the "newer build (HEAD `82c11f6`)" attribution is an **inference** from CI
> artifact-freshness ordering, not an independently verified build-commit
> ascription (no build-id metadata was shipped with the artifact and the runner
> kept only the analysis outputs). Signer identity IS byte-identical, which
> confirms the artifact is a genuine InkFlow release build, but the exact source
> commit of the analyzed binary could not be re-derived after the run.
...
|----|----------|------|----------|------------|---------------|
| R2-KS-01 | INFO | backup | `apktool d` manifest: `android:allowBackup="false"` + `android:fullBackupContent="false"` + `android:dataExtractionRules="@xml/data_extraction_rules"`; `res/xml/data_extraction_rules.xml` excludes the app-private internal-root (`domain="root" path="."`) for both cloud-backup + device-transfer — NOTE: no `external_data`/`device_*` domain exclusions are declared (the app keeps no external app data; vault DB, cache and `voice_notes` are all internal). | `aapt dump xmltree` on the built APK, verify each attr. | None — PASS. |
...
| R2-KS-17 | INFO | plugin cert-pin | **Default pin is still the placeholder** `sha256/AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=` (bytes 0x00–0x1F — can never match a real leaf cert) for host `plugin-updates.inkflow.app` (`m0/C2256b.java:772-773`); fetch host is allowlisted to that single host (`n2/AbstractC2550i:11`); constant-time compare (`o3/J3`, `MessageDigest.isEqual`). Consequence: plugin "update" fetches **fail closed** until a real pin ships — NOT a bypass, but the store's update path is inert-by-default. | `grep "sha256/AAECAwQFBgc"` over dex; trace pin consumption (`o3/C2901j3:19-20`). | Ship a real (or secondary) pin from build config; document pin-rotation runbook. Fix-phase candidate: **phase-171** (round-1 Phase-32-NEW-04 — operator runbook, do-not-substitute-guess-pin). |
...
> **SUPERSEDED (phase-160 review fix):** the bullets below describe the state
> phase-161 observed BEFORE phase-160 was re-unblocked as a STATIC pass. The
> "no findings were recorded / BLOCKED / APK-metadata-only" statements are no
> longer true for this file — the **27 findings (`R2-KS-01..07`, `R2-KS-10..19`,
> `R2-KS-20..29`)** in the phase-160 STATIC section above are the current
> finding set. The phase-161 triage *details* (which round-1 findings map to
> fix phases 170-174) remain valid and are kept below for provenance.

- The Kali dynamic/instrumented pass (phase-160's original dynamic scope) was
  initially BLOCKED on CI: `workspace/phase-160/.blocked` + `.no_work` +
  3 attempts + timeout 360 — no rooted Android device/emulator is available on
  the Linux runner, so no DYNAMIC findings could be produced or appended (they
  are DECLARED `DYNAMIC-DEFERRED` D1-D12 below).
