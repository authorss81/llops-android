...
| `data/db/` | `NoteflowDatabase.kt`, `Daos.kt` | Room DB (schema v9, 8 DAOs), corrupt-DB quarantine; phase-260: NO destructive-migration fallback (fail-closed), `wal_autocheckpoint` callback, busy-inspecting `runWalCheckpointFull`, collision-safe `quarantineSingleFile`, narrowed corruption classifier, paged re-encrypt DAO windows |
| `data/repository/` | `NoteRepository.kt`, `LruBoundedMap.kt` | Encrypted read/write, search corpus, WAL checkpoint, re-key; phase-260: atomic page deletes (`collectPageFilesForDelete` → one-transaction `deletePageRowsPermanently` incl. versions orphan → post-commit file removal), `PageDeleteFilePolicy` confine-on-delete, post-purge VACUUM, all whole-table sweeps paged |
| `services/` | `EncryptionService.kt`, `SecurityService.kt`, `DatabaseSecurityHelper.kt`, `VaultKeyHolder.kt`, `ExportSessionPolicy.kt`, `WetBrushEngine.kt`, `WebDavSyncService.kt`, `ImportExportService.kt`, `ImportArchivePolicy.kt`, `BackupPortabilityPolicy.kt`, `PaletteCatalog.kt`, `ColorModePersistencePolicy.kt`, `ShapeRecognitionHelper.kt`, `SsrfHostPolicy.kt`, `VoiceNoteCrypto.kt`, `DecryptFailurePolicy.kt`, `BrushEdgePolicy.kt`, `PerspectiveGridPolicy.kt`, `TimelapsePolicy.kt`, `TimelapseExporter.kt`, `CanvasStrokeReconcile.kt` | Non-UI: crypto/vault, brush math, sync, import/export, zip-import zip-bomb policy (B1-DB-5), palette, rainbow-mode persistence decision table (phase-122), SSRF blocklist (B1-NET-04), voice-note audio cryptor (B1-DB-3), decrypt-failure render decision (B1-DB-8), brush cap/join roundness policy (phase-121); phase-223 drafting-grid geometry (canvas-rotation math REMOVED in phase-243); phase-224 timelapse timing model + MediaCodec/MediaMuxer MP4 export; phase-252 backup-portability gate (device-keyed export rejection); phase-257 undo/no-resurrect commit-vs-pending-local canvas merge shared by all three AnnotationCanvas ingest effects; phase-260 `PageDeleteFilePolicy` (confine-on-delete for page/voice-blob removal); phase-270 `PluginInstallDefaults.kt` (optional-plugin install defaults) + `PluginPayloadPathPolicy.kt` (payload zip-slip confinement) |
...
| `plugins/store/` | `PluginStoreCatalog.kt`, `PluginStoreController.kt`, `RemotePluginInstaller.kt`, `PluginInstallStore.kt` | Plugin Store lifecycle (bundled catalog + remote install); phase-270: optional ids default NOT installed via pure-JVM `services/PluginInstallDefaults.kt` (`SettingsManager.isPluginInstalledWithDefaults`, explicit key wins), store Delete survives a throwing asset wipe (`code=DELETE_ASSETS_FAILED` + uninstall proceeds), payload extraction zip-slip-confined via pure-JVM `services/PluginPayloadPathPolicy.kt` |
...
- **Glass theme**: `theme/GlassSurfaces.kt:44` (`GlassBlurGate`), :80 (`GlassSurfaceMath`), :140
  ...
  - **Implemented in phase-232** (compile-time source-scan guard, see `workspace/phase-232/REPORT.md`):
    ...
    (28 tests) — a pure-JVM scan over every `app/src/main/kotlin` `.kt` file asserting (1) no
    `verticalScroll` chain places a height bound (`heightIn`/`fillMaxHeight`/`fillMaxSize`/`weight`
    `/height`/`requiredHeight`) AFTER the scroll, (2) no `LazyColumn` nests inside an unbounded
    `verticalScroll` parent, and (3) no height-bound-less `verticalScroll` chain nests inside another
    scrollable (the "nested scrollable without any bound" crash form). Engine `NestedScrollSourceScan`
    (char-level lexer `CodeState` including NESTED block comments, chain walk-back,
    `braceDepthMap`/`nestedUnder` LEXICAL-containment nesting (replaces a fixed 10-line window:
    no sibling-block false positives, no >10-line false negatives), `isInsideBoundProvider` with
    brace-lambda-only shielding for `AlertDialog`/`BasicAlertDialog`/`Dialog`/`ModalBottomSheet`/
    `BoxWithConstraints`) treats dialog/sheet/`BoxWithConstraints`-bound chains and
    `LazyRow`/`horizontalScroll` as the documented known-safe exceptions. The only
    bound-after-scroll in the tree is `BrushStudioDialog.kt:63-65` (AlertDialog text slot —
    parent-bounded), pinned by a dedicated test; all three whole-tree scans are non-vacuity-guarded
    (≥20 real `verticalScroll` sites, ≥5 real `LazyColumn` sites). Compile-stage fixes during the
    phase: `LazyColumn` matched as a bare identifier (`\bLazyColumn\b`); during the review-fix round:
    fixed-height bound vocabulary, brace-depth containment (first attempt used `<=` for the
    closed-scope check and wrongly rejected every parent — corrected to `<`), the unbounded-nested
...
- **Implemented in phase-199** (PERF 2.2+2.3 + review fixes, see `workspace/phase-199/REPORT.md`): baseline-profile
  TOOLCHAIN wired — `:baselineprofile` producer module (`com.android.test` + `androidx.baselineprofile` 1.3.4,
  cold-start and quick-capture→editor→first-stroke macrobenchmarks; deps never enter an APK), consumer plugin on
  `:app`, `androidx.profileinstaller:1.4.1` explicitly pinned, `android.enableR8.fullMode=true`,
  release `isShrinkResources=true`. **No profile is generated/committed yet** (needs a device) — the
  `compileArtProfile` disable is GUARDED (`hasCommittedBaselineProfiles` in `app/build.gradle.kts`): active while
  no real profile is committed, auto-lifts when one lands; run one more release build AFTER
  `generateBaselineProfile`. Review fixes pinned by `Phase199ReleaseShrinkTest`: plugin-sdk consumer rules apply
  GLOBALLY in consumers (never use package wildcards that also match host code — SDK keeps are single-star root
