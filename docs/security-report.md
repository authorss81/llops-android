...
- **Evidence:** `HostedPluginManifest.kt:29-57` (a manifest offer carries `downloadUrl`, `sha256`, AND `pinnedCertHash`), `PluginUpdateChecker` `toTargetEntry` (OLD `:30-38` copied the manifest's `downloadUrl`/`sha256`/`pinnedCertHash` verbatim into the persisted entry; the NEW offer path builds `PluginUpdateInfo` from the compile-time pin values), `PluginUpdateEngine.kt:110-116` (reviewed — verifier used to run against those manifest-supplied values; the engine now gates the persisted target on the compile-time pins first), `HttpsManifestTransport` `PluginManifestFetcher.kt:83-151` (manifest fetched over system-chain HTTPS only — no cert pin, no signature — with `instanceFollowRedirects=true` at line 98), `HttpsPluginDownloadTransport.kt:56-62` (artifact transport also follows redirects)
