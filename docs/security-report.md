...
- **Resolved — phase-66 (commit `llops/phase-66`):** The exploit path is closed. `ArtifactSignatureVerifier.kt` no longer takes a last-signed-entry cert: `collectSignerSet` (`ArtifactSignatureVerifier.kt:152`) force-verifies the JAR (`JarFile(verify=true)`) and requires EVERY non-META-INF entry to carry exactly one certificate (an unsigned entry inside a verified jar — the attacker-key-on-`classes.dex` shape — is a hard `Result.Invalid`), rejects any multi-signer entry (`certs.size != 1`, `:169`), rejects any archive whose entries mix different certs (`sameCert`, `:181`), and fails hard on an EMPTY verified signer set (`:191`) — never a fallback to a last-seen value. New pure-JVM `SignerCertificatePolicy.kt` is the single decision table run by `verify()` (`:97`) BEFORE the pin compare: `checkValidity(now)` rejects an expired/not-yet-valid pinned cert (previously silently accepted) and a `KeyUsage` extension lacking the digitalSignature bit (bit 0) is rejected; an absent extension is unrestricted (RFC 5280). As defense in depth, a key-usage-invalid cert is ALSO refused by the signer-set gate when the platform JAR verifier surfaces such entries with `null` certificates. Pure JVM, runs unchanged on the API 26+ floor (no new platform requirement). Tests: `B1Crypto08SignerSetTest` (12 — single-pinned-signer passes; two-signer jar over every entry fails; signed+unsigned mixed jar fails; unsigned artifact fails; expired cert fails even with a matching pin hash; KeyUsage-excluded cert refused; policy table accepts absent/digitalSignature usage and rejects expired/not-yet-valid/non-signing certs; source pins for `collectSignerSet`/`SignerCertificatePolicy.validate` and absence of the `certs.firstOrNull()` fallback). `gradle testDebugUnitTest` 1287 total (only the 2 pre-existing `B1Plat01ReleaseSigningTest` failures, untouched build.gradle/RELEASE.md asserts documented since phase-55) + `gradle assembleDebug` green (173.7 MB debug APK SHA-256 `41f94289…`). No schema change, no migration, no new deps, `.github/workflows/` untouched.
...
| B1-CRYPTO-08 | MEDIUM | phase-66 | `FIXED` 2026-08-15 (the artifact-signer pin binds the FULL signer certificate set, not a "last signed entry seen" cert, and the pinned cert must be currently valid with digital-signature key usage: `ArtifactSignatureVerifier.collectSignerSet` (`plugins/runtime/ArtifactSignatureVerifier.kt`) force-verifies the JAR and rejects any unsigned non-META-INF entry, any multi-signer entry (`certs.size != 1`), any archive mixing different certs across entries, and an EMPTY verified signer set (never the old `certs.firstOrNull()` last-entry fallback); new pure-JVM `plugins/runtime/SignerCertificatePolicy.kt` is the single decision table that `verify()` runs BEFORE the pin compare — `checkValidity(now)` rejects expired/not-yet-valid certs and a `KeyUsage` extension lacking the digitalSignature bit is rejected (absent extension = unrestricted, RFC 5280); the key-usage-invalid cert is ALSO caught by the signer-set gate when the platform JAR verifier leaves such entries' certs null — see `workspace/phase-66/REPORT.md`, pinned by `B1Crypto08SignerSetTest`, 12 tests) |
